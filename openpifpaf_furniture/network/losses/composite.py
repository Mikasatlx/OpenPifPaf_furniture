...
####################  Composite fields extension due to furniture classification  ###################
        self.n_class = head_meta.n_class
#####################################################################################################
    ...
    def forward(self, x, t):
        ...
        assert x.shape[2] == 1 + self.n_confidences + self.n_class + self.n_vectors * 2 + self.n_scales
        assert t.shape[2] == self.n_confidences + self.n_class + self.n_vectors * 3 + self.n_scales
        ...
        reg_mask = torch.all(finite[:, :, :, :, self.n_confidences+self.n_class:1 + self.n_class + self.n_vectors * 2], dim=4)
        scale_mask = torch.all(finite[:, :, :, :, self.n_confidences+self.n_class + self.n_vectors * 3:], dim=4)

        # extract masked ground truth
        t_confidence_bg = t[bg_mask][:, 0:self.n_confidences]
        #print(t_confidence_bg.size())
        t_confidence = t[c_mask][:, 0:self.n_confidences]

####################  Loss for furniture classification  ############################################
        t_class_valid = t[c_mask][:,self.n_confidences:self.n_confidences+self.n_class]
#####################################################################################################

        t_regs = t[reg_mask][:, self.n_confidences+self.n_class:1+self.n_class + self.n_vectors * 2]
        t_sigma_min = t[reg_mask][
            :,
            self.n_confidences+self.n_class + self.n_vectors * 2:self.n_confidences+self.n_class + self.n_vectors * 3
        ]
        t_scales_reg = t[reg_mask][:, self.n_confidences+self.n_class + self.n_vectors * 3:]
        t_scales = t[scale_mask][:, self.n_confidences+self.n_class + self.n_vectors * 3:]
...
####################  Loss for furniture classification  ############################################
        x_class_valid = x[c_mask][:,1+self.n_confidences:1+self.n_confidences+self.n_class]
#####################################################################################################
        x_logs2_reg = x[reg_mask][:, 0:1]
        x_regs = x[reg_mask][:, 1 + self.n_confidences+self.n_class:1 + self.n_confidences+self.n_class + self.n_vectors * 2]
        x_scales_reg = x[reg_mask][:, 1 + self.n_confidences+self.n_class + self.n_vectors * 2:]
        x_scales = x[scale_mask][:, 1 + self.n_confidences+self.n_class + self.n_vectors * 2:]
...
####################  Loss for furniture classification  ############################################
        l_class = self.ce_loss(x_class_valid, t_class_valid)
#####################################################################################################
        ...
        if self.soft_clamp is not None:
            ...
            l_class = self.soft_clamp(l_class)
...
####################  Loss for furniture classification  ############################################
            torch.sum(0.2*l_class) / batch_size,
#####################################################################################################
