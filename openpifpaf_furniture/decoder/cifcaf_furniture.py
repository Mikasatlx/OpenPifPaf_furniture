...
class CifCafFurniture(openpifpaf.decoder.Decoder):
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        ...
        assert openpifpaf.decoder.utils.nms.Keypoints.get_keypoint_threshold() == CppCifCaf.get_keypoint_threshold()
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        # check consistency
        if args.seed_threshold < args.keypoint_threshold_furniture:
            LOG.warning(
                'consistency: decreasing keypoint threshold to seed threshold of %f',
                args.seed_threshold,
            )
            args.keypoint_threshold_furniture = args.seed_threshold

        cls.nms_before_force_complete = args.nms_before_force_complete_furniture
        openpifpaf.decoder.utils.nms.Keypoints.set_keypoint_threshold(keypoint_threshold_nms)
        ...
        openpifpaf.decoder.utils.CifSeeds.set_ablation_nms(args.ablation_cifseeds_nms_furniture)
        openpifpaf.decoder.utils.CifSeeds.set_ablation_no_rescore(args.ablation_cifseeds_no_rescore_furniture)
        openpifpaf.decoder.utils.CafScored.set_ablation_no_rescore(args.ablation_caf_no_rescore_furniture)
        if args.ablation_cifseeds_no_rescore_furniture and args.ablation_caf_no_rescore_furniture:
            openpifpaf.decoder.utils.CifHr.set_ablation_skip(True)
