...
class MeanPixelError(Base):
    ...
    def accumulate(self, predictions, image_meta, *, ground_truth=None):
        ...
        pck = 100*average(detections_pck)
        LOG.info('Mean Pixel Error (scaled): %s (%s)    Det. Rate (scaled): %s (%s)    PCK: %s',
                 str(mpe)[:4], str(mpe_scaled)[:4], str(det_rate)[:4], str(det_rate_scaled)[:4], str(pck)[:4])
        ...
        self.detections_pck.extend(detections_pck)
    ...
    def stats(self):
        ...
        pck = 100*average(self.detections_pck)
        LOG.info('Final Results: \nMean Pixel Error [scaled] : %f [%f] '
                 '\nDetection Rate [scaled]: %f [%f]'
                 '\nPercentage of Correct Keypoints: %f',
                 mpe, mpe_scaled, det_rate, det_rate_scaled, pck)
        data = {
            'stats': [mpe, mpe_scaled, det_rate, det_rate_scaled, pck],
