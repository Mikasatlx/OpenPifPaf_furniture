...
#include "openpifpaf_furniture/utils.hpp"
...
struct Annotation {
    ...
    Annotation(void) : id(-1), category(-1) { }
    explicit Annotation(int n_joints) : joints(n_joints), id(-1), category(-1) { }
    explicit Annotation(const std::vector<Joint>& joints_) : joints(joints_), id(-1), category(-1) { }
    Annotation(int n_joints, int id_) : joints(n_joints), id(id_), category(-1) { }
    Annotation(const std::vector<Joint>& joints_, int id_) : joints(joints_), id(id_), category(-1) { }
};
//////////////////////////////////////////////////////////////////////////////
...
struct OPENPIFPAF_FURNITURE_API CifCaf : torch::CustomClassHolder {
    ...
    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> call_with_initial_annotations(
        const torch::Tensor& cif_field,
        int64_t cif_stride,
        const torch::Tensor& caf_field,
        int64_t caf_stride,
