...
struct Seed {
    int64_t f;
    float v, v1, v2, v3, v4, x, y, s;

    Seed(int64_t f_, float v_, float v1_, float v2_, float v3_, float v4_, float x_, float y_, float s_)
        : f(f_), v(v_), v1(v1_), v2(v2_), v3(v3_), v4(v4_), x(x_), y(y_), s(s_) { }
};


struct DetSeed {
    int64_t c;
    float v, v1, v2, v3, v4, x, y, w, h;

    DetSeed(int64_t c_, float v_, float v1_, float v2_, float v3_, float v4_, float x_, float y_, float w_, float h_)
        : c(c_), v(v_), v1(v1_), v2(v2_), v3(v3_), v4(v4_), x(x_), y(y_), w(w_), h(h_) { }
};
//====================================================================================================================
