...
#include "openpifpaf_furniture/decoder/cifcaf.hpp"

#include "openpifpaf_furniture/decoder/utils/caf_scored.hpp"
#include "openpifpaf_furniture/decoder/utils/cif_hr.hpp"
#include "openpifpaf_furniture/decoder/utils/cif_seeds.hpp"
#include "openpifpaf_furniture/decoder/utils/nms_keypoints.hpp"
#include "openpifpaf_furniture/decoder/utils/occupancy.hpp"
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> CifCaf::call(
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> CifCaf::call_with_initial_annotations(
    const torch::Tensor& cif_field,
    int64_t cif_stride,
    const torch::Tensor& caf_field,
    int64_t caf_stride,
...
//=============================  Composite fields extension due to furniture classification ==========================
    ...
    for (int64_t seed_i=0; seed_i < seeds_f.size(0); seed_i++) {
        f = seeds_f_a[seed_i];
        x = seeds_vxys_a[seed_i][5];
        y = seeds_vxys_a[seed_i][6];
        s = seeds_vxys_a[seed_i][7];
        ...
        joint.y = y;
        joint.s = s;
        int64_t max_class_i = 1;
        float max_class = 0;
        for(int64_t class_i=1; class_i < 5; class_i++){      // Classification decoding
            if(max_class < seeds_vxys_a[seed_i][class_i]){
                max_class = seeds_vxys_a[seed_i][class_i];
                max_class_i = class_i;
            }
        }
        annotation.category = max_class_i;
//====================================================================================================================
#ifdef DEBUG
...
#endif
//=============================  Composite fields extension due to furniture classification ==========================
    auto out = torch::empty({ int64_t(annotations.size()), n_keypoints, 8 });
    auto out_ids = torch::empty({ int64_t(annotations.size()) }, torch::kInt64);
    auto out_categories = torch::empty({ int64_t(annotations.size()) }, torch::kInt64);
    auto out_a = out.accessor<float, 3>();
    auto out_ids_a = out_ids.accessor<int64_t, 1>();
    auto out_categories_a = out_categories.accessor<int64_t, 1>();  // Classification decoding
    for (int64_t ann_i = 0; ann_i < int64_t(annotations.size()); ann_i++) {
        ...
        out_categories_a[ann_i] = ann.category;  // Classification decoding
    }
    return std::make_tuple(out, out_ids, out_categories);
}
//====================================================================================================================
