...
from ..visualizer import CafFurniture as CafVisualizer
...
class CafFurniture:
    ...
    def __call__(self, image, anns, meta):
        return CafFurnitureGenerator(self)(image, anns, meta)


class AssociationFiller:
    def __init__(self, config: CafFurniture):
        self.config = config
    ...
    def fill_field_values(self, field_i, fij, fill_values, category_id):
        raise NotImplementedError
...
#############################################################################################################
        ...
        self.fill(keypoint_sets, fill_values, category_sets)
    ...
    def fill(self, keypoint_sets, fill_values, category_sets):
        for keypoints, fill_value, category_id in zip(keypoint_sets, fill_values, category_sets):
            self.fill_keypoints(keypoints, fill_value, category_id)
    ...
    def fill_keypoints(self, keypoints, fill_values, category_id):
        for field_i, joint1i, joint2i in self.config.fill_plan:
            ...
            self.fill_association(field_i, joint1, joint2, fill_values, category_id)

    def fill_association(self, field_i, joint1, joint2, fill_values, category_id):
        ...
        for f in frange:
            for xyo in xyv:
                ...
                self.fill_field_values(field_i, fij, fill_values, category_id)


class CafFurnitureGenerator(AssociationFiller):
    def __init__(self, config: CafFurniture):
...
#################   Encode additional composite field for classification    ###############
        ...
        self.class_bed = np.zeros(self.field_shape, dtype=np.float32)
        self.class_chair = np.zeros(self.field_shape, dtype=np.float32)
        self.class_sofa = np.zeros(self.field_shape, dtype=np.float32)
        self.class_table = np.zeros(self.field_shape, dtype=np.float32)
###########################################################################################
...
#################   Encode additional composite field for classification    ###############
        ...
        self.class_table[:, p:-p, p:-p][:, bg_mask == 0] = np.nan
###########################################################################################
...
#################   Encode additional composite field for classification    ###############
        if category_id == 1:
            self.class_bed[field_i, fij[1], fij[0]] = 1
        if category_id == 2:
            self.class_chair[field_i, fij[1], fij[0]] = 1
        if category_id == 3:
            self.class_sofa[field_i, fij[1], fij[0]] = 1
        if category_id == 4:
            self.class_table[field_i, fij[1], fij[0]] = 1
###########################################################################################
...
#################   Encode additional composite field for classification    ###############
        ...
        class_table = self.class_table[:, p:-p, p:-p]
###########################################################################################
...
#################   Encode additional composite field for classification    ###############
        ...
        mask_valid_area(class_table, valid_area)
###########################################################################################
