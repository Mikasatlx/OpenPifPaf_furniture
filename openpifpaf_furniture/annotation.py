...
class Annotation(openpifpaf.annotation.Base):
...
##############################  Get annotation category   ###################################
        self.category_id = None    # Can not just be set to 1
#############################################################################################
    ...
    @classmethod
    def from_cif_meta(cls, cif_meta: headmeta.CifFurniture):
        scale = np.sqrt(
    ...
    def set(self, data, joint_scales=None, *, category_id=None, fixed_score=None, fixed_bbox=None):
    ...
    def inverse_transform(self, meta):
        ...
        if angle != 0.0:
            ...
            if ann.fixed_bbox is not None:
                ann.fixed_bbox = openpifpaf.utils.rotate_box(ann.fixed_bbox, rw - 1, rh - 1, angle)
